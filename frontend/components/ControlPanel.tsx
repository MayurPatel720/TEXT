...
export default function ControlPanel({
  ...
  return (
    <aside className="card p-6 h-full overflow-y-auto" aria-labelledby="controls-heading">
      <div className="mb-6">
        <h3 id="controls-heading" className="font-semibold text-lg">Design Controls</h3>
      ...
      <div className="space-y-6">
        ...
        <div className="control-section">
          ...
          <div className="space-y-2">
            {(["original", "warm", "cool", "monochrome"] as const).map((palette) => (
              <div
                key={palette}
                className={`card p-3 cursor-pointer transition-all ${params.colorPalette === palette ? "border-[var(--accent)] bg-[var(--accent-glow)]" : "hover:border-[var(--border-hover)]"}`}
        ...
        {/* Motif Size */}
        <div className="control-section">
          <label className="label" htmlFor="motif-size-slider">
            Motif Size
        ...
        {/* Density */}
        <div className="control-section">
          <label className="label">Pattern Density</label>
          <div className="space-y-2">
            {(["sparse", "normal", "dense"] as const).map((density) => (
              <div
                key={density}
                className={`card p-3 cursor-pointer transition-all ${params.density === density ? "border-[var(--accent)] bg-[var(--accent-glow)]" : "hover:border-[var(--border-hover)]"}`}
        ...
        <div className="control-section">
          ...
          <div className="space-y-2">
            {(["normal", "mirrored", "rotated"] as const).map((orientation) => (
              <div
                key={orientation}
                className={`card p-3 cursor-pointer transition-all ${params.orientation === orientation ? "border-[var(--accent)] bg-[var(--accent-glow)]" : "hover:border-[var(--border-hover)]"}`}
